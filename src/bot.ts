...
bot.command("start", async (ctx) => {
  await ctx.reply(
    "Olá! Eu sou o Cuiudobô, seu assistente pessoal de IA. Como posso ajudar você hoje?",
  );
});
...
bot.command("clear", async (ctx) => {
  ...
  await ctx.reply(
    "Minha memória do nosso contexto foi limpa. Começaremos do zero! 🧹",
  );
});
...
bot.on("message:text", async (ctx) => {
  ...
  // Filtro básico de segurança antes de processar no LLM
  if (detectPromptInjection(text)) {
    logger.warn(`Possível Prompt Injection detectada de ${userId}`, { text });
    await ctx.reply(
      "⚠️ Minha heurística de segurança bloqueou sua mensagem porque ela se assemelha a uma injeção de comandos de sistema.",
    );
    return;
  }
...
bot.on("message:document", async (ctx) => {
  ...
  if (fileSize > MAX_SIZE) {
    await ctx.reply(
      `O arquivo é muito grande (${(fileSize / 1024 / 1024).toFixed(2)}MB). O limite é 10MB.`,
    );
    return;
  }
